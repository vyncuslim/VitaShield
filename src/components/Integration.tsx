...
<script src="https://cdn.vitamind.ai/shield/v1/widget.js" async defer></script>
...
app.post('/login', async (req, res) => {
  ...
  const response = await fetch('https://api.vitamind.ai/v1/verify', {
    method: 'POST',
...
def verify_shield_token(token, client_ip):
    url = "https://api.vitamind.ai/v1/verify"
    payload = {
        "secret": "${keys.secretKey}",
        "token": token,
        "ip": client_ip
...
func verifyToken(token, ip string) (bool, error) {
    ...
    resp, err := client.Post("https://api.vitamind.ai/v1/verify", "application/json", bytes.NewBuffer(reqBody))
    if err != nil {
        return false, err
    }
...
public class ShieldVerifier {
    ...
    public static boolean verifyToken(String token, String ip) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://api.vitamind.ai/v1/verify"))
...
?>`,
    ...
    curl: `curl -X POST https://api.vitamind.ai/v1/verify \\
      ...
      <div style={styles.layout}>
        ...
        <div className="glass-panel" style={styles.guidePanel}>
          ...
          <div style={styles.postmanBox}>
            ...
            <button 
              onClick={() => handleCopy('https://api.vitamind.ai/v1/postman_collection.json', 'postman')} 
